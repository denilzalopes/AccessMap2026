  ...
  return (
    <div className="space-y-8">
      ...
      <div className={`rounded-xl bg-gradient-to-r ${scoreColor} p-8 text-white text-center shadow`}>
        ...
        <p className="mt-2 text-7xl font-bold">{Number(esg.sustainabilityScore ?? 0).toFixed(0)}</p>
      ...
      <div className="bg-rebijoux-teal/5 border border-rebijoux-teal/20 rounded-lg p-5">
        <p className="text-sm font-medium text-rebijoux-teal">
          Your portfolio represents <span className="font-bold">{Number(esg.totalRecycledGoldGrams ?? 0).toFixed(2)} grams</span> of recycled gold —
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <ImpactCard emoji="🌳" label="Forest Preservation" value={`${Number(esg.forestSavedHectares ?? 0).toFixed(3)} hectares`} description="Deforestation prevented through recycled gold use" color="border-rebijoux-green" />
        <ImpactCard emoji="⚗️" label="Mercury Reduction" value={`${Number(esg.mercuryAvoidedKg ?? 0).toFixed(3)} kg`} description="Mercury pollution avoided vs. primary mining" color="border-rebijoux-teal" />
